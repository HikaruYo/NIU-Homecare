...
@section('content')
    <div class="flex flex-col w-full h-full gap-4 bg-mainGray shadow-lg rounded-lg">
        <div class="mx-6 py-4 border-b-2 border-gray-200">
            <h1 class="text-4xl font-semibold text-gray-800">Edit Layanan</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui detail layanan agar informasi tetap sesuai.</p>
        </div>
        ...
        <div class="px-6 pb-6 flex-1 min-h-0 overflow-y-auto">
            <div class="bg-white shadow rounded-2xl p-6 md:p-8">
                ...
                <form enctype="multipart/form-data" method="POST"
                    action="{{ route('admin.dashboard.layanan.update', $layanan->layanan_id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="nominal" value="{{ old('nominal', $layanan->nominal) }}"
                                class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-gray-800 focus:outline-none focus:ring-2 focus:ring-mainColor">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="hargaPer30Edit" type="number" name="harga_per_30menit"
                    ...
                    <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                        ...
                        <div class="flex items-center gap-6">
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                                <input id="flexYesEdit" type="radio" name="is_flexible_duration" value="1"
                                    class="text-mainColor focus:ring-mainColor"
                                    {{ old('is_flexible_duration', $layanan->is_flexible_duration) == 1 ? 'checked' : '' }}>
                                Ya
                            </label>
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                                <input id="flexNoEdit" type="radio" name="is_flexible_duration" value="0"
                                    class="text-mainColor focus:ring-mainColor"
                        ...
                        @if ($layanan->gambar)
                            <div class="mt-4">
                                ...
                                <div class="bg-gray-50 border border-gray-200 rounded-xl p-3 inline-block">
                                    <img src="{{ asset('storage/' . $layanan->gambar) }}" alt="Preview"
                                        class="h-52 w-auto rounded-lg object-cover">
                                </div>
                            </div>
                        @endif
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const flexYes = document.getElementById('flexYesEdit');
            const flexNo = document.getElementById('flexNoEdit');
            const hargaPer30 = document.getElementById('hargaPer30Edit');

            if (!flexYes || !flexNo || !hargaPer30) return;

            const baseEnabledClass =
                'w-full rounded-xl border border-gray-300 px-4 py-2.5 text-gray-800 focus:outline-none focus:ring-2 focus:ring-mainColor bg-white';
            ...
            const updateHargaPer30State = () => {
                ...
                hargaPer30.disabled = !isFlexible;
                hargaPer30.className = isFlexible ? baseEnabledClass : baseDisabledClass;
            };
